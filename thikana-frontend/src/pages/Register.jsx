...
const Register = () => {
  ...
  const validateForm = () => {
    ...
    if (role !== 'agency') {
      if (!nidNumber) {
        showNotification(
          'NID number is required for Tenant and Owner accounts',
          'warning'
        );
        return false;
      }
      ...
      if (!nidRegex.test(nidNumber)) {
        showNotification(
          'NID number must be exactly 10, 13, or 17 digits',
          'warning'
        );
        return false;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showNotification(
        err.message || 'Registration failed. Email or phone might already be registered.',
        'error'
      );
    } finally {
      setSubmitting(false);
    ...
    <div className="min-h-[80vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-2xl w-full bg-white p-8 sm:p-10 rounded-3xl border border-slate-100 shadow-xl space-y-8">
        <div className="text-center">
          <div className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-600 text-white font-bold text-2xl shadow-lg shadow-emerald-500/20 mb-4">
            T
          </div>
          ...
          <p className="mt-2 text-sm text-slate-400">
            Join the smart rental management platform
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="text-center pt-2">
            <p className="text-sm text-slate-400">
              ...
              <Link
                to="/login"
                className="inline-flex items-center gap-0.5 font-bold text-emerald-600 hover:text-emerald-700"
              >
                Sign in here
